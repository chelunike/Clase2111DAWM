...
public class Fecha1Josemi {
    private  int dia, mes, año;
    
    public Fecha1Josemi(){}
    
    
    public Fecha1Josemi(int dia,int mes,int año){
        this.dia=dia;
        this.mes=mes;
        this.año=año;
    }
    //public static void leerFecha(){}
    public Fecha1Josemi (String fecha){
        String dd, mm, aa;
        if (fecha.length()==10){
            ...
            if ((fecha.charAt(2)=='-'||fecha.charAt(2)=='/')
                ...
                if(Fecha1Josemi.esDij(dd) && Fecha1Josemi.esDij(mm) && Fecha1Josemi.esDij(aa)){
    ...
    public boolean comprobar(){
        ...
        if (dia > 0 && dia<=31){
            if(mes<=12 && mes>0){
                if (año>=1582){
                    if (Fecha1Josemi.diaMes(mes)>=dia){
                        c = true;
                    }
                }
            }
        ...
        if(mes==2){
            if(Fecha1Josemi.esBisiesto(año)&& dia>0 && dia<=29)
                c = true;
        }
        return c;
    }
    ...
    public int diferenciaDias(Fecha1Josemi f2){
        ...
        int xd=0;
            if(ai != af){
            xd += Fecha1Josemi.diaMes(mi) - di;
            xd += df;
            for(int i=mi;i<=12;i++){
                xd += Fecha1Josemi.diaMes(i);
            }
            ai++;
            for(int k=0;k<mf;k++){
                xd+= Fecha1Josemi.diaMes(k);
            }
            for(int j=ai;j<af;j++){
                if (Fecha1Josemi.esBisiesto(j)){
                    xd+=366;
                }else{
                    xd+=365;}
            }
    ...
    public int difAño(Fecha1Josemi f1, Fecha1Josemi f2){
    ...
    public int difaño(Fecha1Josemi f2){
    ...
    public static int difMes( Fecha1Josemi f1, Fecha1Josemi f2){
    ...
    public boolean mayorFecha(Fecha1Josemi f2) {
        boolean c = false;
    ...
    public boolean menorFecha(Fecha1Josemi f){
        return !(this.mayorFecha(f));
    }
    
    public void intercambia(Fecha1Josemi f){
    ...
    public static void intercambia(Fecha1Josemi f1, Fecha1Josemi f2){
