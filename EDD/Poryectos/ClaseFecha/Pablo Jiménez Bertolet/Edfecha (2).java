...
public class Fecha2 {
    ...
    public Fecha2(String s) {
        int longi = s.length();
        if (longi != 10) //caracteres que tiene que tener 
        ...
        for (int i = 0; i < longi; i++) {
            char c = s.charAt(i);
            if (i == 2 || i == 5) //esto sirve para que la fecha solo admita el guin o la barra (- ó /)
            {
                if (c != '-' && c != '/') {
                    return;
                } else {
...
return false;
    ...
    private int diasMes() {
        int[] tdias = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}; //puede ser asi o con llaves|| int[] tdias = new int [12];
        //el primer valor esta en la posicion 0 y la ultima en la 12
