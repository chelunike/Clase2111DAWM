...
public class Ventanuco extends JFrame implements ActionListener {
    ...
    private MatriculaPanel matriculaJf;
    ...
    public Ventanuco(Contenedor cont){
        ...
        matriculaJf = new MatriculaPanel(Alumno.Asignaturas);
        ...
        matriculaJf.setBounds(300, 20, 250, 300);
    ...
    public void escribeAsigNota(){
        ...
        int lon=0;
        AsigNota[] notas;
        if(al1.getNotas()[0] != null){
            lon = al1.getNotas().length;
            notas = al1.getNotas();
        }else{
            lon = 5;
            notas = new AsigNota[lon];
        }
            
        for(int i=0; i<al1.getNotas().length; i++){
            JTextField tmp = new JTextField(al1.getNotas()[0]==null?"":notas[i].getAsig());
            tmp.setHorizontalAlignment(0);
            tmp.setBackground(Color.lightGray);
            tmp.setBounds( 13, 30*i+50, 100, 20);
            notasPn.add(tmp);
            JTextField tmp2 = new JTextField(al1.getNotas()[0]==null?"":""+notas[i].getNota());
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource().equals(insert)){
            ...
            try{
                ...
                //Compruebo las asignturas matriculadas--------------------------------------------Nuevo :)
                AsigNota[] notas;
                if(matriculaJf.leerAsiganturas().length == 0)
                    throw new Exception(){
                        @Override
                        public String toString(){
                            return "no asignaturas";
                        }
                    };
                else{
                    String[] asignaturas = matriculaJf.leerAsiganturas();
                    notas = new AsigNota[asignaturas.length];
                    int n=0;
                    for(String asig: asignaturas){
                        notas[n] = new AsigNota(asig,0);
                        n++;
                    }
                    matriculaJf.setBackground(Color.white);
                }
                
                //ArreFinikitao lo creo e inserto
                Alumno al1 = new Alumno(nom, fechaL, curs, notas);
                ...
                if(ex.toString().compareToIgnoreCase("nombre en blanco")==0)
                    ...
                    matriculaJf.setBackground(Color.red);
            }finally{
                for(int i=0; i<listaAlumnos.size(); i++)
                    System.out.println(""+listaAlumnos.getPersona(i).getNombre());
            }
        }
