...
public class InsertarPanel /*extends JDialog*/ implements ActionListener{
    //Atributos
    private final Ventanuco padre; //Version Antigua
    private Alumno hijo;
    ...
    //Constructor
    public InsertarPanel(Ventanuco pae){
        padre = pae;
        init();
    }
    ...
    public void insertarAlumnoNuevo(){
        padre.setBotonesEnabled(false);
        ventanita.setVisible(true);
        System.out.println("Arre cojio er nuevo Alumno :) ");
        if(hijo != null)
            padre.getContenedor().insertarPersona(hijo);
        padre.setBotonesEnabled(true);
        padre.actualizaAlumno();
    }
    
    public void modificarAlumno(Alumno actual){
        padre.setBotonesEnabled(false);
        ventanita.setVisible(true);
        if(hijo != null){
            actual.setNombre(hijo.getNombre());
            actual.setFechaNaci(hijo.getFechaNaci());
            actual.setNotas(hijo.getNotas());   
        }
        padre.setBotonesEnabled(true);
        padre.actualizaAlumno(); 
    }
