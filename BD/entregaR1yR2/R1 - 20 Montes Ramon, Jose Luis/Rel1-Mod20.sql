...
('04/05/2005', '23:00',	'Avnda Dilar',	'Conducir en dirrecion contraria',	5996.12, 55447788,	'5678def'),
...
('09/11/2009', '12:48',	'Calle Santa Rosalia',	'Exceso de velocidad',	300,	55447788, '5678def'),
...
-- Personas que han sufrido mas de un accidente
select * from persona
	where dni in (select persona from accidentados 
						group by persona
						having COUNT(*)>1)

-- coches con mas de un propietario
select * from vehiculo
	where matricula in (select vehiculo from tiene
				group by vehiculo
				having COUNT(*)>1)
				
-- Personas con mas de un vehiculo
select * from persona 
	where dni in (select persona from tiene
					group by persona
					having COUNT(*)>1)

-- Acciendetes con mas de una persona involucrada
select * from accidente
	where numRef in (select accidente from accidentados
						group by persona, accidente
						having COUNT(*)>1)
...
-- Numero de multas de cada persona
select dni, nombre,COUNT(*) Multas  from persona left join multa
	on dni = persona
	group by dni, nombre
...
select dni, nombre,
	COUNT(*) Multas,
	SUM(importe) Total_Deber  
	from persona left join multa
	on dni = persona
	group by dni, nombre

-- Personas con importes totales de las multas superiores a la media
select dni, nombre
	from persona left join multa
	on dni = persona
	group by dni, nombre
	having SUM(importe)(select AVG(importe) from multa)

-- Funcion para obtener una lista de accidentes a patir de una persona(dni)
create function listadoIncidencias(@dni int)
returns table as return
	select * from accidente inner join accidentados
		on numRef = accidente
		where persona = @dni

select * from listadoIncidencias(55447788)

-- Procedimiento para obtener un listado top 3 de personas que tengan mas accidentes y multas
create procedure personasPeligrosas as 
begin 
	select top 3 *,
		(select COUNT(*) from accidentados
			where persona = P.dni
			group by persona)+
		(select COUNT(*) from multa
			where persona = P.dni
			group by persona) numIncidencias
		from persona
		order by numIncidencias desc
end

exec personasPeligrosas